...
**Approach #1**: Local port forwarding:
```
###############
# On server `A`
###############
...
**Approach #2**: Remote port forwarding
> **NOTE**: It's exactly the same, but the command is started on the other end of the tunnel
```
###############
# On server `B`
###############
# Establish SSH tunnel
ssh -R <PORT_A>:<IP_Y>:<PORT_Y> <IP_A>
...
Looks at each individual packet that goes through the FW. 

Makes a decision, that based on the **information, contained within that individual packet**.

> You can lose a big picture, looking at the individual packet. Thus, other (more sophisticated) types of FW exist.


### Stateful FW


Looks at the set of packets all together. 

Makes a decision, looking at the set of packets (not inididual ones).

For example, connection:
- if this packet belongs to this connection, then I can let you go through;
- but if the packet is not part of existing connection, I wont.

So decision for individual packet is made, **based on the information from previous packet**.



### Application FW

Packet and Statefule FW mostly look at the header part of the packet (IP header, TCP/UDP header), but not at the data part.


Application FW look at the data part, so - application layer.

Makes a decision, based on the information from `Application Layer 7`.

The more information you look at, the more computing resources you're gonna use.

> **NOTE**: From performance perspective, more simpler FW are more effective.  


## Overall

Once packet is on the computer, FW should look at its header (IP, TCP/UDP, for example) and decides, whether drop the packet or let it move further.

At userspace (where `App` are located):
- you work with data part of the packet, not its headers;
- you not gonna control how packet flows.

Thus, seems like FW needs to be inside of `OS kernel` -> to be part of `OS kernel`. In this case, we would have to modify the `OS kernel`, which is complicated, dangerous and would make a kernel messy.

So Linux community came up with following.

They developed a foundation for you to build FW. 

It's called a `Netfilter`.


## Netfilter Hooks

So instead of modifying `OS kernel`, `Netfilter` provides some places in the `OS kernel` that are a hooks (placeholders) for your code.

In this way you have ability to make any steps with packet, in the following points:
1) `NF_INET_PRE_ROUTING` - when incoming packet just arrived to the machine (after network interface card, but before packet routing);
2) `NF_INET_FORWARD` - if incoming packet is not for this machine (after packet routing, when it's ready to be forwarded further in case of router);
3) `NF_INET_LOCAL_IN` - if incoming packet is for this machine, right before providing it to the `App` (so after packet routing, when it's ready to be forwarded to `Network Stack`);
4) `NF_INET_LOCAL_OUT` - if new packet is just generated by the `App` on the machine (so after `Network Stack`, before routing);
5) `NF_INET_POST_ROUTING` - if originaly incoming or newly generated packet ready to be sent out of the machine (so the final point, where something could be done to the packet, just before it is given to the network interface card).
 

> **NOTE**: Ability to insert any code into a placeholders is that what makes `Netfilter` so usefule not only for FW, but other app as well.

Because any code can be implemented within a placeholders, `Netfilter` is useful for:
- packet filtering (stateless firewall);
- connection tracking (stateful firewall);
- packet adress translation (NAT: as we can change source IP, for example);
- further packet modifications (Load Balancing?)

> **NOTE**: We cannot insert application code inside the hooks, provided by `Netfilter`. This code must be inside the `OS kernel`.

In old days, if you need to add a new code to the kernel, you have to recompile the kernel.

But this days Linux allows you to dynamically load your code to the kernel - that's called `loadable kernel module`:
- code of kernel module is written;
- the code has to be compiled then (to be applicable for current kernel);
- the compiled code of kernel module has to be inserted into the kernel:
```
# Insert the kernel module into running kernel
sudo insmod kMod.ko
...
# See results as a message
dmesg
```

## iptables

> **NOTE**: As `iptables` is built on top of `Netfilter Hooks`, it can do more then just a FW (for example, NAT).

`iptables` consist of 2 parts:
- iptables (which is in userspace)
- kernel space code:
  - the kernel modules, that are hooked into `Netfilter Hooks` inside the `OS kernel`;

Those 2 parts communicate.

Once you inserted the rule into `iptables` (which is in userspace), it's gonna be inserted into the kernel modules (which is in kernel space) and eventually hooked to `Netfilter`.

So `iptables` just help you to configure `Netfilter Hooks` in more convenient way.

To do that, `iptables` organize rules into following tables:
- filter - packet filtering;
- nat - modifying src/dest network adress;
- mangle - packet content modification (for example, TTL within the header).
- other tables ...

Each table has its own `chains` (like `INPUT`, `FORWARD`, `OUTPUT`, etc).
